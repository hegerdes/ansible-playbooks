...
async function generateNginxConf(){
  ...
  for(let site of ingress.sites){
    ...
    fs.writeFileSync(`${file_dst_name}`, res)
  }

  // Default site
  let create_default_server = (ingress.conf && ingress.conf.nginx && ingress.conf.nginx.create_default_server) ? ingress.conf.nginx.create_default_server : 'yes'
