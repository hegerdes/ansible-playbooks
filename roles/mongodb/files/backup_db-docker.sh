...
docker exec mongodb sh -c 'exec mongodump -u=$MONGO_INITDB_ROOT_USERNAME -p=$MONGO_INITDB_ROOT_PASSWORD --archive' | gzip > ${BACKUP_DIR}/${DATE_STR}-${HOSTNAME}.dump.tar.gz
